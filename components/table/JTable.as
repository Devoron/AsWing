...
package org.aswing
{
	...
	//import devoron.gamestudio.tools.guibuilder.DSOCellEditor;
	...
	public class JTable extends Container implements Viewportable, TableModelListener, TableColumnModelListener, CellEditorListener, LayoutManager
	{
		...
		private var scrollPaneDisabledWhenEditing:Boolean;
		...
		public function JTable(dm:TableModel = null)
		{
			...
			scrollPaneDisabledWhenEditing = true;
		...
		public function setScrollPaneDisabledWhenEditing(b:Boolean):void
		{
			if (scrollPaneDisabledWhenEditing == b)
				return;
			scrollPaneDisabledWhenEditing = b;
			
			if (b)
			{
				if (getParent() is JScrollPane)
				{
					addEventListener(TableCellEditEvent.EDITING_CANCELED, onEditingCancel);
					addEventListener(TableCellEditEvent.EDITING_STARTED, onEditingStart);
					addEventListener(TableCellEditEvent.EDITING_STOPPED, onEditingStop);
				}
			}
			else
			{
				removeEventListener(TableCellEditEvent.EDITING_CANCELED, onEditingCancel);
				removeEventListener(TableCellEditEvent.EDITING_STARTED, onEditingStart);
				removeEventListener(TableCellEditEvent.EDITING_STOPPED, onEditingStop);
			}
		
		}
		
		public function isScrollPaneDisabledWhenEditing():Boolean
		{
			return scrollPaneDisabledWhenEditing;
		}
		...
		private function onAdded(e:Event):void
		{
			if (getParent() is JScrollPane)
			{
				if (scrollPaneDisabledWhenEditing)
				{
					addEventListener(TableCellEditEvent.EDITING_CANCELED, onEditingCancel);
					addEventListener(TableCellEditEvent.EDITING_STARTED, onEditingStart);
					addEventListener(TableCellEditEvent.EDITING_STOPPED, onEditingStop);
		...
		private function onEditingCancel(e:TableCellEditEvent):void
		{
			var vsb:JScrollBar = (getParent() as JScrollPane).getVerticalScrollBar();
			if (vsb)
				vsb.setEnabled(true);
		}
		...
		private function onEditingStart(e:TableCellEditEvent):void
		{
			var vsb:JScrollBar = (getParent() as JScrollPane).getVerticalScrollBar();
			if (vsb)
				vsb.setEnabled(false);
				
				//this.addEventListener(MovedEvent.MOVED, onTableMoved);
				/*trace(this);
				...
				p.addEventListener(DragAndDropEvent.DRAG_RECOGNIZED, onDragRecognized);*/
		...
		private function onTableMoved(e:MovedEvent):void 
		{
			trace(e);
		}
		...
		public function setShowHorizontalLines(showHorizontalLines:Boolean, affectCellSpacing:Boolean = true):void
		{
			...
			if (affectCellSpacing)
			{
				setIntercellSpacing(new IntDimension(getIntercellSpacing().width, showHorizontalLines ? 1 : 0));
			}
		...
		public function setShowVerticalLines(showVerticalLines:Boolean, affectCellSpacing:Boolean = true):void
		{
			...
			if (affectCellSpacing)
			{
				setIntercellSpacing(new IntDimension(showVerticalLines ? 1 : 0, getIntercellSpacing().height));
			}
		...
		protected function changeSelectionModel(sm:ListSelectionModel, index:int, toggle:Boolean, extend:Boolean, selected:Boolean, programmatic:Boolean = true):void
		{
			if (extend)
			{
				if (toggle)
		...
		public function setColumnsSize(... sizes):void
		{
			var columnsCount:int = getColumnCount();
			for (var i:int = 0; i < columnsCount; i++)
			{
					getColumnAt(i).setPreferredWidth(sizes[i]);
					getColumnAt(i).setMinWidth(sizes[i]);
					getColumnAt(i).setMaxWidth(sizes[i]);
					getColumnAt(i).setWidth(sizes[i]);
			}
		}
		
		public function setColumnsSizes(sizes:Array):void
		{
			var columnsCount:int = getColumnCount();
			//var columnsCount:int = sizes.length;
			for (var i:int = 0; i < columnsCount; i++)
			{
					getColumnAt(i).setPreferredWidth(sizes[i]);
					getColumnAt(i).setMinWidth(sizes[i]);
		...
		public function setColumnsResizable(value:Boolean):void
		{
			var columnsCount:int = getColumnCount();
			for (var i:int = 0; i < columnsCount; i++)
				getColumnAt(i).setResizable(value);
		}
		...
		public function __listSelectionChanged(e:SelectionEvent):void
		{
			...
			dispatchEvent(new SelectionEvent(SelectionEvent.ROW_SELECTION_CHANGED, e.getFirstIndex(), e.getLastIndex(), e.isProgrammatic()));
			resizeAndRepaint();
		}
		...
		public function editingStopped(source:CellEditor):void
		{
			...
			/*	if (editor is DSOCellEditor) {
					if ((editor as DSOCellEditor).close){
							removeEditor();
					}
				}
				else {*/
		...
		{
			//var value:Object = _storedValue
				setValueAt(_storedValue, editingRow, editingColumn);
				/*editor
...
//		editorRemover = null;
		...
		private function layoutCells():void
		{
			...
			headerPane.setComBoundsXYWH(insetsX, insetsY, cWidth, headerHeight);
			
			//layout table header
			if (getTableHeader())
			{
				getTableHeader().setLocationXY(-viewPosition.x, 0);
				getTableHeader().setSizeWH(getLastTotalColumnWidth(), headerHeight);
			...
			if (fotr)
			{
				fotr.setComBoundsXYWH(insetsX, getHeight() - insets.bottom - fotr.getPreferredHeight(), cWidth, fotr.getPreferredHeight());
				fotr.validate();
			...
			if (getTableHeader())
				...
				cellPaneBounds.setLocation(new IntPoint(insetsX, insetsY));
...
//        hasFocus = (rowIsLead && colIsLead) && isFocusOwner();
		...
		private function synCreateCellInstances():void
		{
			...
			else if (rowCells.length > needNum)
			{ //remove mored
