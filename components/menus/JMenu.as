...
package org.aswing
{
	...
	public class JMenu extends JMenuItem implements MenuElement
	{
		...
		public function addMenuItemsListener(listener:Function, priority:int = 0, useWeakReference:Boolean = false):void
		{
			if (!popupMenu)
				return;
			var menuItems:Array = popupMenu.getSubElements();
			for each (var item:JMenuItem in menuItems)
			{
				item.addActionListener(listener, priority, useWeakReference);
			}
		}
		
		public function removeMenuItemsListener(listener:Function):void
		{
			if (!popupMenu)
				return;
			var menuItems:Array = popupMenu.getSubElements();
		...
		public function addMenuItems(... titles):void
		{
			for (var i:int = 0; i < titles.length; i++)
			{
				addMenuItem(titles[i]);
			}
		}
