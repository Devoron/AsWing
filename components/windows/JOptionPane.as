...
package org.aswing
{
	...
	public class JOptionPane extends JPanel
	{
		...
		public function JOptionPane()
		{
			...
			msgLabel = new JLabel();
			msgLabel.setForeground(new ASColor(0xFFFFFF, 0.8));
		...
		public function getCancelButton():JButton
		{
			if (cancelButton == null)
			{
				...
				cancelButton.setMaximumHeight(16);
			}
			return cancelButton;
		}
		...
		private function addCloseListener(button:JButton):void
		{
			...
			button.addActionListener(function():void
				{
					f.tryToClose();
				});
		}
		...
		public static function showMessageDialog(title:String, msg:String, finishHandler:Function = null, parentComponent:Component = null, modal:Boolean = true, icon:Icon = null, buttons:int = OK):JOptionPane
		{
			...
			(frame.getTitleBar() as JFrameTitleBar).setClosableOnly(true);
			//frame.getBackgroundChild().alpha = 0;
			frame.setIcon(icon);
			//frame.setBackgroundDecorator(new ColorDecorator(new ASColor(0x262F2B, 1), new ASColor(0xFFFFFF, 0.4)));
			
			var id:ColorDecorator = new ColorDecorator(new ASColor(0x1C221F, 1), new ASColor(0XFFFFFF, 0.24), 4);
			id.setGaps(-2, 1, 1, -2);
			//id.setGaps(-1, 0, 0, -1);
			frame.setBackgroundDecorator(id);
		...
		{
			var frame:JFrame = new JFrame(AsWingUtils.getOwnerAncestor(parentComponent), title, modal);
			frame.getTitleBar().setRestoreButton(null);
			frame.getTitleBar().setMaximizeButton(null);
			frame.getTitleBar().setIconifiedButton(null);
			frame.getTitleBar().getSelf().buttonMode = true;
			frame.getTitleBar().getCloseButton().buttonMode = true;
			frame.getBackgroundChild().alpha = 0;
			frame.setIcon(icon);
			frame.setBackground(new ASColor(0x0E1012, 1));
			...
			pane.inputText.addActionListener(function():void
				{
					if (handler != null)
						handler(pane.getInputText().getText());
				});
