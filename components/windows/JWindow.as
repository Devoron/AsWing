	...
	public class JWindow extends JPopup
	{
		...
		override public function paintImmediately():void
		{
			if (isVisible())
			{
				...
				var needSize:IntDimension = new IntDimension(Math.max(getWidth(), minimizSize.width), Math.max(getHeight(), minimizSize.height));
				setSize(needSize);
		...
		public function setLocationRelativeTo(c:Component = null):void
		{
			if (c == null && stage)
			{
				setLocation(new IntPoint(stage.stageWidth * .5 - getWidth() * .5, stage.stageHeight * .5 - getHeight() * .5));
			}
			else if (c)
			{
				var loc:IntPoint = c.getGlobalLocation();
				var locCenter:IntPoint = new IntPoint(loc.x + c.getWidth() * .5 - getWidth() * .5, loc.y + c.getHeight() * .5 - getHeight() * .5);
				setLocation(locCenter);
			}
		}
		...
		public function focusAtThisWindow():void
		{
			...
			if (defaultFocus == null || !(AsWingUtils.isAncestor(this, defaultFocus) && defaultFocus.isShowing() && defaultFocus.isFocusable() && defaultFocus.isEnabled()))
			{
