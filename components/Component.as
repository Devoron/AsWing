...
package org.aswing.components
{
	...
	public class Component extends AWSprite
	{
		...
		{
			var newPos:flash.geom.Point = parent.globalToLocal(new Point(gp.x, gp.y));
		...
		private function getPaintBoundsInRoot():IntRectangle
		{
			...
			if (paintBounds.width < minSize.width)
				paintBounds.width = minSize.width;
			if (paintBounds.height < minSize.height)
				paintBounds.height = minSize.height;
		...
		{
			dispatchEvent(new FocusKeyEvent(FocusKeyEvent.FOCUS_KEY_DOWN, e.charCode, e.keyCode, e.keyLocation, e.ctrlKey, e.altKey, e.shiftKey));
		}
		
		internal function fireFocusKeyUpEvent(e:KeyboardEvent):void
		{
			dispatchEvent(new FocusKeyEvent(FocusKeyEvent.FOCUS_KEY_UP, e.charCode, e.keyCode, e.keyLocation, e.ctrlKey, e.altKey, e.shiftKey));
		}
