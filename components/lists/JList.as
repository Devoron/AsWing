	...
	public class JList extends Container implements LayoutManager, Viewportable, ListDataListener
	{
		...
		public function setAutoDragAndDropType(type:int):void
		{
			...
			if (isAutoDragAndDropAllown())
			{
				...
				addDragAcceptableInitiator(this);
				addEventListener(DragAndDropEvent.DRAG_RECOGNIZED, ____onDragRecognized);
				addEventListener(DragAndDropEvent.DRAG_DROP, ____onDragDrop);
				addEventListener(DragAndDropEvent.DRAG_ENTER, ____onDragEnter);
				addEventListener(DragAndDropEvent.DRAG_EXIT, ____onDragExit);
			}
			else
			{
				setDropTrigger(false);
				setDragEnabled(false);
				removeDragAcceptableInitiator(this);
				removeEventListener(DragAndDropEvent.DRAG_RECOGNIZED, ____onDragRecognized);
				removeEventListener(DragAndDropEvent.DRAG_DROP, ____onDragDrop);
				removeEventListener(DragAndDropEvent.DRAG_ENTER, ____onDragEnter);
				removeEventListener(DragAndDropEvent.DRAG_EXIT, ____onDragExit);
			}
		}
		...
		private function layoutWhenShareCells():void
		{
			...
			// if orientation is vertical, increment y-position
			...
			// if orientation is horizonatal, increment y-position while y lower container height
			// next column - increment x-position
		...
		private var dndAutoScrollTimer:ASTimer;
		...
		private function __onDragRecognized(dragInitiator:Component, touchedChild:Component):void
		{
			if (isAutoDnDInitiatorAllown())
			{
				...
				var sourceData:ListSourceData = new ListSourceData("ListSourceData", selectedIndices);
				...
				DragManager.startDrag(this, sourceData, new ListDragImage(this, offsetY, selectedIndices));
			}
		}
		...
		{
			trace("(sd is ListSourceData) " + (sd is ListSourceData));
			trace("isDragAcceptableInitiator(dragInitiator) " + isDragAcceptableInitiator(dragInitiator));
		...
		{
			trace("checkStartDnDAutoScroll");
			if (dndAutoScrollTimer == null)
			{
				dndAutoScrollTimer = new ASTimer(200);
				dndAutoScrollTimer.addActionListener(__dndAutoScroll);
			}
			...
			if (!dnd_line_mc)
			{
				dnd_line_mc = new MovieClip();
				stage.addChild(dnd_line_mc);
			}
		}
		...
		private function checkStopDnDAutoScroll():void
		{
			...
			if (dnd_line_mc != null)
			{
				dnd_line_mc.graphics.clear();
				dnd_line_mc = null;
			}
		}
		
		private function __dndAutoScroll(e:AWEvent):void
		{
			var lastCellBounds:Rectangle = getCellByIndex(getLastVisibleIndex()).getCellComponent().getBounds(this);
			var firstCellBounds:Rectangle = getCellByIndex(getFirstVisibleIndex()).getCellComponent().getBounds(this);
		...
		private function drawInsertLine():void
		{
			...
			trace("first insert index " + insertIndex);
			if (insertIndex < 0)
			{
				ib = getCellByIndex(lastIndex).getCellComponent().getBounds(this);
				insertIndex = lastIndex + 1;
				insertY = ib.y + ib.height;
			}
			
			trace("second insert index " + insertIndex);
			...
			trace("ins " + ins);
			trace("insertY " + insertY);
			dnd_line_mc.graphics.lineStyle(2, 0x80FF00);
			//dnd_line_mc.graphics.moveTo(ins.left+1, insertY)
			dnd_line_mc.graphics.moveTo(bounds.x + ins.left + 1, bounds.y + insertY)
			//dnd_line_mc.graphics.lineTo(getWidth() - ins.right - 1, insertY);
			dnd_line_mc.graphics.lineTo(bounds.x + ins.left + 1 + getWidth() - ins.right - 1, bounds.y + insertY);
		...
		private function ____onDragRecognized(e:DragAndDropEvent):void
		{
			...
			DragManager.startDrag(this, sourceData, new ListDragImage(this, offsetY, selectedIndices), new DnDHadler());
		}
		
		private function ____onDragEnter(e:DragAndDropEvent):void
		{
			//__onDragEnter(source, dragInitiator, sourceData, mousePos);
			var sourceData:SourceData = e.getSourceData();
			var dragInitiator:Component = e.getDragInitiator();
			//trace("dragInitiator " + dragInitiator);
			dndInsertPosition = -1;
			if (!(isAcceptableListSourceData(dragInitiator, sourceData) && isAutoDnDDropTargetAllown()))
			{
				//trace("Хрень 1");
				//trace(" isAutoDnDDropTargetAllown() "+  isAutoDnDDropTargetAllown());
				//trace("isAcceptableListSourceData(dragInitiator, sourceData)" + isAcceptableListSourceData(dragInitiator, sourceData));
				DragManager.getCurrentDragImage().switchToRejectImage();
			}
			else
			{
		...
		private function ____onDragExit(e:DragAndDropEvent):void
		{
			//__onDragExit(source, dragInitiator, sourceData, mousePos);
			checkStopDnDAutoScroll();
		}
		
		private function ____onDragDrop(e:DragAndDropEvent):void
		{
...
internal class DnDHadler implements DragListener
{
	...
	/*var targetComponent:Component = e.getTargetComponent();
	   ...
	   сt.removeDragAcceptableInitiator(dragInitiator);
	   }
	   else
	   {
