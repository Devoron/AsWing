	...
	public class BasicComboBoxUI extends BaseComponentUI implements ComboBoxUI
	{
		...
		protected function getPopup():JPopup
		{
			if (popup == null)
			{
				...
				//popup.setBackgroundDecorator(new ColorBackgroundDecorator(new ASColor(0x000000, 0.14), null /*new ASColor(0XFFFFFF, 0)*/, 4));
				//popup.setBackgroundDecorator(new ColorBackgroundDecorator(new ASColor(0x000000, 0.14), null /*new ASColor(0XFFFFFF, 0)*/, 4));
				var cb:ColorDecorator = new ColorDecorator(new ASColor(0x0F1E1C, 1), new ASColor(0xFFFFFF, 0.04), 4);
				cb.setGaps(-1, 0, 0, -2);
				popup.setBackgroundDecorator(cb);
			}
			return popup;
		}
		...
		protected function viewPopup():void
		{
			...
			var width:int = box.getPopupListWidth();
			if (width == 0)
				width = box.getWidth();
