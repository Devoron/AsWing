package devoron.aswing3d.plaf.basic
{
	...
	public class BasicSplitPaneUI extends SplitPaneUI implements LayoutManager
	{
		...
		override public function resetToPreferredSizes(jc:JSplitPane):void
		{
			...
			loc = Math.max(getMinimumDividerLocation(), Math.min(loc, getMaximumDividerLocation()));
		...
		public function layoutWithLocation(location:int):void
		{
			...
			else if (location == int.MAX_VALUE)
			{
				...
				if (isVertical())
				{
					divider.setComBoundsXYWH(rect.x, rect.y + rect.height - dvSize, rect.width, dvSize);
					if (lc)
					{
						lc.setComBoundsXYWH(rect.x, rect.y, rect.width, rect.height - dvSize);
					}
				}
				else
				{
					divider.setComBoundsXYWH(rect.x + rect.width - dvSize, rect.y, dvSize, rect.height);
					if (lc)
					{
						lc.setComBoundsXYWH(rect.x, rect.y, rect.width - dvSize, rect.height);
					}
				}
				if (lc)
					lc.setVisible(true);
			...
			else
			{
				...
				if (isVertical())
				{
					divider.setComBoundsXYWH(rect.x, rect.y + location, rect.width, dvSize);
					if (lc)
						lc.setComBoundsXYWH(rect.x, rect.y, rect.width, location);
					if (rc)
						rc.setComBoundsXYWH(rect.x, rect.y + location + dvSize, rect.width, rect.height - location - dvSize);
				}
				else
				{
					divider.setComBoundsXYWH(rect.x + location, rect.y, dvSize, rect.height);
					if (lc)
						lc.setComBoundsXYWH(rect.x, rect.y, location, rect.height);
					if (rc)
						rc.setComBoundsXYWH(rect.x + location + dvSize, rect.y, rect.width - location - dvSize, rect.height);
				}
				if (lc)
					lc.setVisible(true);
				if (rc)
		...
		{
			return Math.max(getMinimumDividerLocation(), Math.min(loc, getMaximumDividerLocation()));
		}
		...
		protected function __div_mouse_moving(e:MouseEvent):void
		{
			if (!sp.isContinuousLayout())
			{
				...
				var newPoint:flash.geom.Point = newGlobalPos.toPoint();
		...
		protected function getCurrentMovedDistance():int
		{
			...
			newLocation = Math.max(getMinimumDividerLocation(), Math.min(newLocation, getMaximumDividerLocation()));
		...
		public function preferredLayoutSize(target:Container):IntDimension
		{
			...
			if (isVertical())
			{
				size = new IntDimension(Math.max(lcSize.width, rcSize.width), lcSize.height + rcSize.height + getDividerSize());
			}
			else
			{
				size = new IntDimension(lcSize.width + rcSize.width + getDividerSize(), Math.max(lcSize.height, rcSize.height));
			}
