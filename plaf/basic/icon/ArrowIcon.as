...
public class ArrowIcon implements Icon, UIResource{
	...
	public function updateIcon(c:Component, g:Graphics2D, x:int, y:int):void{
		...
		var center:flash.geom.Point = new flash.geom.Point(c.getWidth()/2, c.getHeight()/2);
		var w:Number = width - 6;
		var ps1:Array = new Array();
		ps1.push(nextPoint(center, arrow, w/2/2));
		var back:flash.geom.Point = nextPoint(center, arrow + Math.PI, w/2/2);
		...
		if(c is AbstractButton){
	    	...
	    	if(!b.isEnabled()){//disabled
	    		...
	    		cl = cl.offsetHLS(offHue-hue, 0.1, 0.1);
	    	}
		}
	...
	protected function nextPoint(p:flash.geom.Point, dir:Number, dis:Number):flash.geom.Point{
		return new flash.geom.Point(p.x+Math.cos(dir)*dis, p.y+Math.sin(dir)*dis);
	}
