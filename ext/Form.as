...
package devoron.aswing3d.ext
{
	...
	public class Form extends JPanel implements LayoutManager
	{
		...
		private var textRenderer:Class;
		...
		public function setTextRenderer(cl:Class):void
		{
			this.textRenderer = cl;
		}
		
		public function getTextRenderer():Class
		{
			return textRenderer;
		}
		...
		public function addLeftHoldRow(gap:int, ... comps):FormRow
		{
			...
			for (var i:int = 0; i < comps.length; i++)
			{
				...
				if (obj is Array)
				{
					comps.splice(i, 1, new JSp((obj as Array)[0], (obj as Array)[1]));
				}
			}
		...
		public function addCenterHoldRow(gap:int, ... comps):FormRow
		{
			...
			for (var i:int = 0; i < comps.length; i++)
			{
				var obj:* = comps[i];
				if (obj is int)
					comps.splice(i, 1, new JSp(obj as int));
				if (obj is String)
				{
					var textElement:* = new textRenderer;
					textElement.setText(obj);
			...
			p.setPreferredWidth(super.getWidth());
			for each (var c:Component in comps)
				p.append(c);
		...
		public function addRightHoldRow(gap:int, ... comps):FormRow
		{
		...
		public function createRow(columns:Array):FormRow
		{
			...
			for (var i:int = 0; i < columns.length; i++)
			{
				...
				if (obj is int)
				{
					columns.splice(i, 1, new JSp(obj));
				}
				if (obj is String)
				{
					var textElement:* = new textRenderer;
					textElement.setText(obj);
					columns.splice(i, 1, textElement);
				}
				if (obj is Array)
				{
					columns.splice(i, 1, new JSp((obj as Array)[0], (obj as Array)[1]));
				}
			}
